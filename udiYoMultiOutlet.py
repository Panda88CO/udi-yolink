...
class udiYoMultiOutlet(udi_interface.Node):
    ...
    def start(self):
        ...
        for port in range(0,nbrOutlets):
            try:
                node = udiYoSubOutlet(self.poly, self.address, self.address+str(port+1), 'SubOutlet'+str(port+1))
        ...
        if nbrOutlets == 4: #controllable USB included
            try:
                node = udiYoSubUSB(self.poly, self.address, self.address+str(nbrOutlets+1), 'UsbOutput')
                self.poly.addNode(node)
                self.wait_for_node_done()
            except Exception as e:
                logging.error('Failed to create {}: {}'.format(e))
