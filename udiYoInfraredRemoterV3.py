...
class IRcode(udi_interface.Node):
    ...
    def  __init__(self, polyglot, primary, address, name, yoAccess, IRremote):
...
class udiYoInfraredRemoter(udi_interface.Node):
    ...
    def start(self):
        ...
        active_codes = self.yoIRrem.get_active_code_dict ()
        for code in active_codes:
            address = self.poly.getValidAddress((self.primary[-13:]+'_'+str(code)))
            name = self.getalidName('code '+str(code))
            self.ircode_nodes[code] = IRcode(self.poly, self.primary, address, name,  self.yoAccess, self.yoIRrem)
