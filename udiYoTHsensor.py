    ...
    def updateStatus(self, data):
        ...
        if self.node is not None:
            if self.online:
                ...
                self.node.setDriver('CLITEMP', self.yoTHsensor.getTempValueC(), True, True)
                self.node.setDriver('-1', self.yoTHsensor.bool2Nbr(alarms['lowTemp']), True, True)
                self.node.setDriver('-1', self.yoTHsensor.bool2Nbr(alarms['highTemp']), True, True)
                self.node.setDriver('CLIHUM', self.yoTHsensor.getHumidityValue(), True, True)
                self.node.setDriver('-1', self.yoTHsensor.bool2Nbr(alarms['lowHumidity']), True, True)
                self.node.setDriver('-1', self.yoTHsensor.bool2Nbr(alarms['highHumidity']), True, True)
                self.node.setDriver('-1', self.yoTHsensor.getBattery(), True, True)
                self.node.setDriver('-1', self.yoTHsensor.bool2Nbr(alarms['lowBattery']), True, True)
                self.node.setDriver('GV8', self.yoTHsensor.bool2Nbr(self.yoTHsensor.getOnlineStatus()), True, True)
